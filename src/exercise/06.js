...
function useControlledWarning(onIsControlled) {
  if (process.env.NODE_ENV === 'production') return 
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const {current: originalOnIsControlled} = React.useRef(onIsControlled)
  // eslint-disable-next-line react-hooks/rules-of-hooks
  React.useEffect(() => {
    warning(
      !(!onIsControlled && originalOnIsControlled),
      'First: A component is changing an uncontrolled toggle to be controlled. This is likely caused by the value changing from undefined to a defined value, which should not happen. Decide between using a controlled or uncontrolled toggle element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components',
    )
    warning(
      !(onIsControlled && !originalOnIsControlled),
      'Second: A component is changing an uncontrolled toggle to be controlled. This is likely caused by the value changing from undefined to a defined value, which should not happen. Decide between using a controlled or uncontrolled toggle element for the lifetime of the component. More info: https://reactjs.org/link/controlled-components',
    )
  }, [onIsControlled, originalOnIsControlled])
}

function useReadOnlyWarning(onIsControlled, onChange, readOnly) {
  if (process.env.NODE_ENV === 'production') return
  // eslint-disable-next-line react-hooks/rules-of-hooks
  React.useEffect(() => {
    warning(
      !(onIsControlled && onChange === undefined && !readOnly),
      'Failed prop type: You provided an `on` prop to useToggle without an `onChange` handler. This will render a read-only toggle. If the toggle should be mutable use `defaultValue`. Otherwise, set either `onChange` or `readOnly`.',
    )
...
function useToggle({
  ...
  useControlledWarning(onIsControlled)
  useReadOnlyWarning(onIsControlled, onChange, readOnly)
